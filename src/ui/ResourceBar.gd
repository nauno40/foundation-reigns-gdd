...
# colonne 58px (fond, remplissage dégradé, liseré lumineux en crête, icône
# texture PNG, pip) + label 9px en dessous.
...
const ICONS := {
	"military": preload("res://assets/icons/military.png"),
	"religion": preload("res://assets/icons/religion.png"),
	"commerce": preload("res://assets/icons/commerce.png"),
	"politics": preload("res://assets/icons/politics.png"),
}
...
func update_value(value: int) -> void:
	...
	_value_tween.tween_method(_set_display, _display_value, target, 0.28) \
		.set_ease(Tween.EASE_OUT).set_trans(Tween.TRANS_CUBIC)
	_value_tween.finished.connect(func():
...
func _draw() -> void:
	...
	var pct := clampf(_display_value / 100.0, 0.0, 1.0)
	...
	var icon_color := Color(1, 1, 1, 0.55)
	if _affected:
		icon_color = Color(0.918, 0.988, 1.0, 1.0)
	var cx := w / 2.0
	var cy := COL_H / 2.0
	_draw_icon(cx, cy, icon_color)
...
func _draw_icon(cx: float, cy: float, color: Color) -> void:
	var tex: Texture2D = ICONS.get(resource_key)
	if not tex:
		return
	var sz := tex.get_size() * 0.5
	var rect := Rect2(cx - sz.x * 0.5, cy - sz.y * 0.5, sz.x, sz.y)
